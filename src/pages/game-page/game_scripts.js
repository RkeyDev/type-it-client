(function () {
        ...
        try {
            ...
            switch (response.type) {
                ...
                case "countdown_start":
                    startSynchronizedCountdown(response.data);
                    break;
                case "timer_start":
                    startSynchronizedTimer(response.data);
                    break;
    ...
    // =============================
    function startSynchronizedCountdown(data) {
        const { startAt, duration } = data;
        const now = Date.now();
        const diff = startAt - now;

        setTimeout(() => {
            const container = getOrCreateCenter();
            startCountdown(container, () => {
                console.log("Countdown finished — waiting for round start.");
            });
        }, Math.max(0, diff));
    }

    // =============================
    // SERVER-SYNCHRONIZED ROUND TIMER
    // =============================
    function startSynchronizedTimer(data) {
        const { startAt, duration } = data;
        ...
        let timeLeft = Math.max(0, Math.ceil((duration - elapsed) / 1000));
    ...
    // Regular visual countdown (used by the synced version)
    ...
    function startNewRound(question) {
        ...
        if (timerLabel) timerLabel.textContent = roomSettings.typingTime || 60;
